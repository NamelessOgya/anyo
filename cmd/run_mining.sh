...
echo "Running Active Learning Mining in $C..."
docker exec "$C" bash -c "PYTHONPATH=/workspace /opt/conda/bin/poetry run python -m src.utils.mine_hard_examples --config-name mining $*"
