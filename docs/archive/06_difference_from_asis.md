...
*   **差分と再現性評価**:
    ...
    *   **補足**: 損失計算におけるアイテムIDのインデックス処理（1-based vs 0-based）について、本プロジェクトではPyTorchの標準的な慣習および`SASRecTrainer`との整合性を重視し、損失関数への入力前に0-basedに統一する処理を明示的に実装しました。これにより、参照実装で潜在的に発生しうるインデックスエラーや不整合を回避しています。
