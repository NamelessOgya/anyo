...
def parse_args():
    ...
    parser.add_argument(
        "--filter_sizes", nargs="?", default="[2,3,4]", help="Specify the filter_size"
    )
    # dro
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="weight for collaborative embedding distillation",
    )
    parser.add_argument(
        "--lam",
        type=float,
        default=0.8,
        help="weight for importance-aware ranking distillation",
    )
    parser.add_argument(
        "--candidate_topk", type=int, default=10, help="top k items from llm"
    )
    parser.add_argument(
        "--gamma_position",
        type=float,
        default=0.3,
        help="weight for ranking position-aware",
    )
    parser.add_argument(
        "--gamma_confidence",
        type=float,
        default=0.5,
        help="weight for ranking importance-aware",
    )
    parser.add_argument(
        "--gamma_consistency",
        type=float,
        default=0.1,
        help="weight for ranking consistency-aware",
    )
    parser.add_argument(
        "--beta2",
        type=float,
...
def myevaluate(model, test_data, device, llm_all_emb=None):
    ...
    prediction = model.forward(
        states, np.array(len_states), llm_emb
    )  # [num_test,num_item]
    ...
    ndcg_list = []
    print(
        "hr@{}\tndcg@{}\thr@{}\tndcg@{}\thr@{}\tndcg@{}\thr@{}\tndcg@{}".format(
            topk[0], topk[0], topk[1], topk[1], topk[2], topk[2], topk[3], topk[3]
        )
    )
...
if __name__ == "__main__":
    ...
    if model_name == "GRU":
        ...
        model = SASRec(
            args.hidden_factor, item_num, seq_size, args.dropout_rate, device
        )
    ...
    for i in range(args.epoch):
        ...
        for j in range(num_batches):
            ...
            zeros_tensor[
                torch.arange(real_batch_size).unsqueeze(1).repeat(1, 10), seq
            ] = 1
            ...
            # dros loss
            if args.alpha != 0:
                pos_scores_dro = torch.gather(
                    torch.mul(model_output * model_output, ps), 1, target
                )
                pos_scores_dro = torch.squeeze(pos_scores_dro)
                pos_loss_dro = torch.gather(
                    torch.mul((model_output - 1) * (model_output - 1), ps), 1, target
                )
                pos_loss_dro = torch.squeeze(pos_loss_dro)
                inner_dro = (
                    torch.sum(
                        torch.exp(
                            (torch.mul(model_output * model_output, ps) / args.beta)
                        ),
                        1,
                    )
            ...
            if args.lam != 0:
                ...
                weight_rank = weight_static / torch.sum(weight_static, dim=1).unsqueeze(
                    1
                )
                weight_rank = weight_rank.to(device)
                # weight_com
                cf_rank_top = (
                    (-model_output).argsort(dim=1)[:, :_K].to(device)
                )  # candidate [1024, k]
                ...
                weight_confidence = weight_confidence / torch.sum(
                    weight_confidence, dim=1
                ).unsqueeze(1)
                weight_confidence = weight_confidence.to(device)
                # weight_fin
                weight_fin = (
                    args.gamma_position * weight_rank
                    + args.gamma_confidence * weight_confidence
                    + args.gamma_consistency * weight_com
                )
                weight = weight_fin / torch.sum(weight_fin, dim=1).unsqueeze(
                    1
                )  # [1024,k]
                # distillation loss
                loss_all_rd = 0
                ...
                for i_ in range(num_candidate):
                    ...
                    neg_labels = torch.zeros((args.batch_size, 1)).to(
                        device
                    )  # [1024,1]
                    loss_bce_rd = -(
                        pos_labels * torch.log(torch.sigmoid(pos_scores))
                        + (1 - neg_labels) * torch.log(torch.sigmoid(1 - neg_scores))
                    )
                    if args.alpha != 0:
                        # dro loss
                        pos_scores_dro = torch.gather(
                        ...
                        pos_loss_dro = torch.gather(
                            torch.mul((model_output - 1) * (model_output - 1), ps),
                            1,
                            target,
                        )
                        ...
                        # all loss
                        loss_all_rd += (
                            weight[:, i_ : i_ + 1] * loss_bce_rd
                        ).mean() + args.alpha * (
                            weight[:, i_ : i_ + 1] * loss_dro_rd
                        ).mean()
                    else:
        ...
        if True:
            ...
            e_epoch = time.time()
            print(
                f"the loss in {step}th step is: {loss_all}, train this epoch cost {e_epoch-s_epoch} s"
            )
            if step % 1 == 0:
                print("VAL PHRASE:")
                val_path = os.path.join(data_directory, "val_data.csv")
                prediction, hr_list, ndcg_list = myevaluate(
                    model, val_path, device, llm_all_emb
                )
                print("TEST PHRASE:")
                test_path = os.path.join(data_directory, "test_data.csv")
                s_test = time.time()
                prediction, hr_list, ndcg_list = myevaluate(
                    model, test_path, device, llm_all_emb
                )
                e_test = time.time()
                if ndcg_list[-1] > best_ndcg20:
                    ...
                    patient += 1
                print(
                    f"patient={patient}, BEST STEP:{best_step}, BEST NDCG@20:{best_ndcg20}, BEST HR@20:{best_hr20}, test cost:{e_test-s_test}s"
                )
