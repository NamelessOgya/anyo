...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA, PeftType.MOELORA):
        ...
        if bias == "none":
            to_return = {
                k: state_dict[k] for k in state_dict if "lora_" in k or "gating" in k
            }
        elif bias == "all":
            to_return = {
                k: state_dict[k]
                for k in state_dict
                if "lora_" in k or "bias" in k or "gating" in k
            }
        elif bias == "lora_only":
            to_return = {}
            for k in state_dict:
        ...
        to_return = {
            ...
            if (
                ("lora_" in k and adapter_name in k) or ("bias" in k) or ("gating" in k)
            )
        }
