...
class MoeLoraConfig(PeftConfig):
    ...
    gating: str = field(
        default="Standard",
        metadata={"help": "Select the gating network for each MoeLora"},
    )
    ...
    def _check_target_module_exists(self, moelora_config, key):
        if isinstance(moelora_config.target_modules, str):
            ...
            target_module_found = any(
                key.endswith(target_key) for target_key in moelora_config.target_modules
            )
            is_using_layer_indexes = (
                getattr(moelora_config, "layers_to_transform", None) is not None
            )
                ...
                for pattern in layers_pattern:
                    ...
                    if layer_index is not None:
                        ...
                        if isinstance(moelora_config.layers_to_transform, int):
                            target_module_found = (
                                layer_index == moelora_config.layers_to_transform
                            )
                        else:
                            target_module_found = (
                                layer_index in moelora_config.layers_to_transform
                            )
        ...
        for key in key_list:
            ...
            if isinstance(target, MoeLoraLayer) and isinstance(target, torch.nn.Conv2d):
                ...
                new_module = self._create_new_module(
                    moelora_config, adapter_name, target
                )
...
class MoeLoraLayer:
    ...
    def update_layer(
        ...
        self.gating.update(
            nn.ModuleDict(
                {
                    adapter_name: GATING_TO_MODEL_MAPPING[gating](
                        num_moe=num_moe, dim=self.in_features
                    )
                }
            )
        )
        ...
        def forward(self, x: torch.Tensor):
            ...
            if self.disable_adapters or self.active_adapter not in self.lora_A.keys():
                ...
                if not torch.is_autocast_enabled():
                    ...
                    B_out = self.calculate_B(A_out)
                    Gate = self.gating[self.active_adapter](user_embeds).unsqueeze(-1)
                    output = (B_out * Gate).sum(dim=-2).to(
                        expected_dtype
                    ) * self.scaling[self.active_adapter]
                    ...
                    B_out = self.calculate_B(A_out)
                    Gate = self.gating[self.active_adapter](user_embeds).unsqueeze(-1)
                    output = (B_out * Gate).sum(dim=-2) * self.scaling[
                        self.active_adapter
                    ]
            ...
            def forward(self, x: torch.Tensor):
                ...
                if (
                    ...
                    if not torch.is_autocast_enabled():
                        ...
                        B_out = self.calculate_B(A_out)
                        Gate = self.gating[self.active_adapter](user_embeds).unsqueeze(
                            -1
                        )
                        output = (B_out * Gate).sum(dim=-2).to(
                            expected_dtype
