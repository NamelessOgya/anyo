...
class MInterface(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        targets = targets.masked_fill(
            (batch["tokens"].token_type_ids == 0)[:, 1:], -100
        )
    ...
    def save_gradients_to_file(self):
        ...
        file_path = os.path.join(
            directory, f"gradients_step_{self.trainer.global_step}.pkl"
        )
    ...
    def training_step(self, batch, batch_idx):
        if self.scheduler:
            self.scheduler.step(
                self.trainer.global_step, self.current_epoch, self.trainer.max_steps
            )
        if batch["flag"]:
        ...
        if self.hparams.llm_tuning == "lora":
            if self.hparams.peft_dir:
                self.llama_model = PeftModel.from_pretrained(
                    self.llm_model, self.hparams.peft_dir, is_trainable=True
                )
            else:
                if self.hparams.peft_config:
                    peft_config = LoraConfig(
                        **LoraConfig.from_json_file(self.hparams.peft_config)
                    )
                else:
                    peft_config = LoraConfig(
            ...
            if self.hparams.peft_dir:
                ...
                if self.hparams.peft_config:
                    peft_config = MoeLoraConfig(
                        **MoeLoraConfig.from_json_file(self.hparams.peft_config)
                    )
                else:
                    peft_config = MoeLoraConfig(
                        task_type=TaskType.CAUSAL_LM,
    ...
    def wrap_emb(self, batch):
        input_embeds = self.llama_model.get_input_embeddings()(
            batch["tokens"].input_ids
        )

        his_token_id = self.llama_tokenizer(
            "[HistoryEmb]", return_tensors="pt", add_special_tokens=False
        ).input_ids.item()
        cans_token_id = self.llama_tokenizer(
            "[CansEmb]", return_tensors="pt", add_special_tokens=False
        ).input_ids.item()
        item_token_id = self.llama_tokenizer(
            "[ItemEmb]", return_tensors="pt", add_special_tokens=False
        ).input_ids.item()
        ...
        for i in range(len(batch["len_seq"])):
            if (batch["tokens"].input_ids[i] == his_token_id).nonzero().shape[0] > 0:
                idx_tensor = (
                    (batch["tokens"].input_ids[i] == his_token_id).nonzero().view(-1)
                )
                for idx, item_emb in zip(
                    idx_tensor, his_item_embeds[i, : batch["len_seq"][i].item()]
                ):
                    input_embeds[i, idx] = item_emb
            if (batch["tokens"].input_ids[i] == cans_token_id).nonzero().shape[0] > 0:
                idx_tensor = (
                    (batch["tokens"].input_ids[i] == cans_token_id).nonzero().view(-1)
                )
                for idx, item_emb in zip(
                    idx_tensor, cans_item_embeds[i, : batch["len_cans"][i].item()]
                ):
                    input_embeds[i, idx] = item_emb
