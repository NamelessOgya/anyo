...
class LinearWarmupCosineLRScheduler:
    def __init__(
        ...
        **kwargs
        ...
        self.warmup_start_lr_list = (
            warmup_start_lr_list if warmup_start_lr_list is not None else init_lr_list
        )

    def step(self, cur_step, cur_epoch, max_step):
        for i, param_group in enumerate(self.optimizer.param_groups):
            if cur_epoch == 0 and cur_step < self.warmup_steps:
                lr = self.warmup_lr_schedule(
                    cur_step, self.warmup_start_lr_list[i], self.init_lr_list[i]
                )
            else:
                lr = self.cosine_lr_schedule(
    ...
    def cosine_lr_schedule(self, step, max_step, init_lr, min_lr):
        ...
        lr = (init_lr - min_lr) * 0.5 * (
            1 + math.cos(math.pi * step / max_step)
        ) + min_lr
        return lr
    ...
    def warmup_lr_schedule(self, step, init_lr, max_lr):
        ...
        lr = min(
            max_lr, init_lr + (max_lr - init_lr) * step / max(self.warmup_steps, 1)
        )
        return lr
