    ...
    def __call__(self, batch):
        if isinstance(self.prompt_list, list):
            ...
            inputs_text = (
                ...
                else [instruction] * len(batch)
            )
        else:
            instruction = (
                sample["instruction_input"] if "instruction_input" in sample else None
            )
            inputs_text = (
                instruction
                if isinstance(instruction, list)
        ...
        if p < thresh_hold or not self.train:
            for i, sample in enumerate(batch):
                ...
                if "[HistoryHere]" in input_text:
                    insert_prompt = ", ".join(
                        [
                            seq_title + " [HistoryEmb]"
                            for seq_title in sample["seq_name"]
                        ]
                    )
                    input_text = input_text.replace("[HistoryHere]", insert_prompt)
                if "[CansHere]" in input_text:
                    insert_prompt = ", ".join(
                        [can_title + " [CansEmb]" for can_title in sample["cans_name"]]
                    )
                    input_text = input_text.replace("[CansHere]", insert_prompt)
                if "[TargetHere]" in input_text:
                    insert_prompt = insert_prompt = ", ".join(
                        [sample["correct_answer"] + " [ItemEmb]"]
                    )
            ...
            for i, sample in enumerate(batch):
                ...
                if "[HistoryHere]" in input_text:
                    insert_prompt = ", ".join(
                        [seq_title + " [PH]" for seq_title in sample["seq_name"]]
                    )
                    input_text = input_text.replace("[HistoryHere]", insert_prompt)
                if "[CansHere]" in input_text:
                    insert_prompt = ", ".join(
                        [can_title + " [PH]" for can_title in sample["cans_name"]]
                    )
                    input_text = input_text.replace("[CansHere]", insert_prompt)
        ...
        if self.train:
            targets_text = [
                target_text + self.terminator for target_text in targets_text
            ]
            ...
            new_batch = {
                "tokens": batch_tokens,
                "seq": torch.stack(
                    [torch.tensor(sample["seq"]) for sample in batch], dim=0
                ),
                "cans": torch.stack(
                    [torch.tensor(sample["cans"]) for sample in batch], dim=0
                ),
                "len_seq": torch.stack(
                    [torch.tensor(sample["len_seq"]) for sample in batch], dim=0
                ),
                "len_cans": torch.stack(
                    [torch.tensor(sample["len_cans"]) for sample in batch], dim=0
                ),
                "item_id": torch.stack(
                    [torch.tensor(sample["item_id"]) for sample in batch], dim=0
                ),
                "flag": flag,
            }
...
class DInterface(pl.LightningDataModule):
    ...
    def __init__(self, llm_tokenizer=None, num_workers=8, dataset="", **kwargs):
        ...
        self.max_steps = (
            self.max_epochs
            * (len(self.trainset) // self.batch_size)
            // self.num_workers
        )
