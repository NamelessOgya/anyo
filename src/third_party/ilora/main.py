...
def load_callbacks(args):
    callbacks = []
    callbacks.append(
        plc.EarlyStopping(monitor="metric", mode="max", patience=10, min_delta=0.001)
    )
...
def main(args):
    ...
    args.max_steps = (
        len(data_module.trainset)
        * args.max_epochs
        // (args.accumulate_grad_batches * args.batch_size)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr_scheduler", default="cosine", choices=["cosine"], type=str
    )
    ...
    parser.add_argument(
        "--llm_path", default="/home/hexngroup/kongxy/llama2_7b_hf", type=str
    )
    parser.add_argument(
        "--rec_model_path", default="LLaRA_MOE/rec_model/SASRec_steam.pt", type=str
    )
    ...
    parser.add_argument(
        "--rec_embed", default="SASRec", choices=["SASRec", "Caser", "GRU"], type=str
    )
    ...
    parser.add_argument(
        "--llm_tuning",
        default="moelora",
        choices=["lora", "freeze", "freeze_lora", "moelora"],
        type=str,
    )
    ...
    parser.add_argument(
        "--router", default="unshare", choices=["share", "unshare"], type=str
    )
