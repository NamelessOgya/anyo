...
from src.core.paths import get_current_run_dir, get_data_dir
from src.core.data_utils import preprocess_data
from src.student.datamodule import StudentDataModule
from src.student.trainer_baseline import train_student_baseline
...
def main(cfg: DictConfig):
    ...
    # Setup logging
    log_file = run_dir / "logs" / "run_student_baseline.log"
    log_file.parent.mkdir(parents=True, exist_ok=True)
    setup_logging(log_file)
    log.info(f"Starting student baseline run in {run_dir}")
    ...
    try:
        ...
        # Train student baseline model
        train_student_baseline(cfg, data_module, tb_logger, run_dir)

        # Evaluate the best baseline model
        log.info("Evaluating the best baseline model on the test set...")
        best_model_path = run_dir / "models" / "baseline" / "best_baseline_model.pt"

        if not best_model_path.exists():
            log.error(
                f"Best baseline model not found at {best_model_path}. Skipping evaluation."
            )
            return
        ...
        metrics_output_path = run_dir / "metrics" / "eval_baseline.json"
        evaluate_model(
            student_model,
        ...
        log.exception("An error occurred during student baseline run:")
        raise
    finally:
        tb_logger.close_all()
        log.info("Student baseline run finished.")
